...
if (!class_exists('YWRAQ_YITH_Composite_Products')) {
	...
	class YWRAQ_YITH_Composite_Products
	{
		...
		public function __construct()
		{
			...
			add_filter('', array($this, 'ajax_add_item'), 10, 2);
						...
						if (isset($component_data['selection_data'][$wcp_key])) {
							...
							?>
								<tr class="yith-composite-child">
									...
									<td scope="col" class="td product-name" style="text-align:left;"><?php echo sprintf('<strong>%s</strong><br>', $component_item['name']); ?>
										<a href="<?php echo $child_product->get_permalink() ?>"><?php echo $title ?></a>
										<?php
										$item_data = array();
									...
									<td scope="col" class="td quantity" style="text-align:left;"><?php echo $child_quantity * $composite_quantity ?></td>
...
<?php
		...
		public function adjust_price($values, $_product, $taxes = 'inc')
		{
			...
			if (isset($values['yith_wcp_component_data']) && 'yith-composite' == $_product->get_type()) {
				...
				$qty       = $values['quantity'];
				$price     = wc_prices_include_tax() ? wc_get_price_including_tax($_product) : wc_get_price_excluding_tax($_product, $qty);
